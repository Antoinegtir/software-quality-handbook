# Project Plan

## Timeline Overview

| Phase | Dates | Who |
|-------|-------|-----|
| Setup (repo, project plan, README) | 20 Mar | Alba |
| Research + Writing — Lead | 21 – 25 Mar | Alba → Task Estimation, Diana → Code Reviews, Antoine → CI/CD |
| Co-author contributes | 26 Mar – 1 Apr | Diana → Task Estimation, Antoine → Code Reviews, Alba → CI/CD |
| Review & PRs | 2 – 3 Apr | Everyone |
| Polish + Retrospective | 4 – 5 Apr | Everyone |

---

## Task Breakdown

### Setup Tasks

| Task | Owner | Branch |
|------|-------|--------|
| Create GitHub repository + Invite collaborators | Antoine | `main` |
| Write README.md | Alba | `1-readme` |
| Write PROJECT_PLAN.md | Alba | `2-project_planmd` |

---

### Research + Writing Tasks (Lead)

| Task | Owner | Branch |
|------|-------|--------|
| Research + write Task Estimation section | Alba | `3-task-estimation` |
| Research + write Code Reviews section | Diana | `4-code-reviews` |
| Research + write CI/CD section | Antoine | `5-cicd` |

> Prioritise articles with **real experiences**, not just theory. Each section should be **500–1000 words** with:
> - Good practices to follow
> - Bad practices to avoid
> - Links to 5 sources
> - At least one diagram or image

---

### Co-author Tasks

| Task | Co-author | Branch |
|------|-----------|--------|
| Contribute to Task Estimation section | Diana | `3-task-estimation` |
| Contribute to Code Reviews section | Antoine | `4-code-reviews` |
| Contribute to CI/CD section | Alba | `5-cicd` |

---

### Review & Merge Tasks

| PR | Reviewer |
|----|----------|
| PR: README | Antoine, Diana |
| PR: Project Plan | Antoine, Diana |
| PR: Task Estimation | Diana, Antoine |
| PR: Code Reviews | Alba, Antoine |
| PR: CI/CD | Diana, Alba |

> Leave **real comments** on each PR.

---

### Final Tasks

| Task | Owner | Branch |
|------|-------|--------|
| Write Retrospective + contributions table | All | `retrospective` |

---

## Individual Contributions


| Team Member | Contribution | Verified by |
|------------|--------------|-----
| Alba | - Created the project plan and assigned tasks to the team <br> - Wrote the README <br> - Created GitHub issues <br> - Researched and wrote the Task Estimation section <br> - Co-authored the CI/CD section | Antoine, Diana |
| Diana | | Alba, Antoine |
| Antoine | | Alba, Diana |

---

## Retrospective

### What went well?
_To be filled in_

### What was challenging?
_To be filled in_

### What would we do differently next time?
_To be filled in_

### Individual reflections

**Alba:**
> I was too optimistic when setting the deadlines in the project plan. I assumed the team would be able to work to the schedule I proposed, but in practice it was very hard to stick to. Most of the work came together in the final days. Next time I would set more realistic timelines and build in buffer, rather than planning for an ideal scenario where everyone delivers on time. That said, the PR review process worked well. Feedback was taken on board and acted on, which meant the content improved noticeably between versions. Despite the tight timeline, the work came together in the end.

**Diana:**
> _To be filled in_

**Antoine:**

The project went well, we encounter no difficulties at all when it comes to use github to organize our group project.
The challenging part was most likely to select the best sources on internet to answer each topics.
Next time I would start peer reviews sooner.